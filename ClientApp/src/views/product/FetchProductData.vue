<template>
  <v-container fluid>
    ...
    <v-alert :value="showError" type="warning">
      ...
      <a href="http://localhost:5000/fetch-product-data">http://localhost:5000</a
      >)
      <br />
      API call would fail with status code 404 when calling from Vue app
      (default at
      <a href="http://localhost:8080/fetch-product-data">http://localhost:8080</a>)
...
export default Vue.extend({
  data() {
    return {
      loading: true,
      showError: false,
      errorMessage: 'Error while loading product data.',
      products: [] as Product[],
      headers: [
        { text: 'Id', value: 'id' },
